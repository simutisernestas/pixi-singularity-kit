...
#
# Notes:
#   - builder itself uses `limactl` and guest-local `/tmp` staging
...
"$builder" --lima-instance "$lima_instance" --output "$kit_root/test/package/package-dev.sif" --manifest "$kit_root/test/package/pyproject.toml"
"$builder" --lima-instance "$lima_instance" --mode pixi-project --output "$kit_root/test/experiment/experiment-pixi.sif" --manifest "$kit_root/test/experiment/pixi.toml"
